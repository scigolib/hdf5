...
func (sb *SuperblockV0Reader) Validate(r io.ReadSeeker) error {
	...
	// Special case: if root group address is 0, try to find the first object header
	if sb.RootGroupAddress == 0 {
		fmt.Println("⚠️  Root group address is 0, searching for first object header...")
		
		// Calculate superblock size for V0
		superblockSize := int64(24 + 5*int(sb.SizeOfOffsets)) // Basic fields + 5 addresses
		
		// Look for OHDR signature after superblock
		if _, err := r.Seek(8+superblockSize, io.SeekStart); err != nil {
			return fmt.Errorf("cannot seek past superblock: %v", err)
		}
		
		// Search for OHDR in next 1024 bytes
		buffer := make([]byte, 1024)
		n, err := r.Read(buffer)
		if err != nil && err != io.EOF {
			return fmt.Errorf("cannot read data after superblock: %v", err)
		}
		
		// Find OHDR signature
		ohdrSignature := []byte{'O', 'H', 'D', 'R'}
		for i := 0; i <= n-4; i++ {
			if buffer[i] == ohdrSignature[0] && 
			   buffer[i+1] == ohdrSignature[1] && 
			   buffer[i+2] == ohdrSignature[2] && 
			   buffer[i+3] == ohdrSignature[3] {
				
				actualRootGroupAddr := uint64(8 + superblockSize + int64(i))
				fmt.Printf("✅ Found root group at offset 0x%016X\n", actualRootGroupAddr)
				
				// Update the root group address
				sb.RootGroupAddress = actualRootGroupAddr
				return nil
			}
		}
		
		return fmt.Errorf("root group address is zero and no OHDR found after superblock")
	}
...
func main() {
	...
	// Validate addresses
	if err := sb.Validate(file); err != nil {
